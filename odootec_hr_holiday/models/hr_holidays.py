...
class HrHolidays(models.Model):
    ...
    vacation_id = fields.Many2one(comodel_name="hr.leave.allocation", string="Vacation",
                                  required=False, )
            ...
            if emp.eligible and emp.holiday_line_ids:
                for line in emp.holiday_line_ids:
                    if line.allocation_range == 'month':
                        ...
                        date_from = datetime.strftime(str(date_from_dt), '%Y-%m-%d')
                        date_to = datetime.strftime(str(date_to_dt), '%Y-%m-%d')
