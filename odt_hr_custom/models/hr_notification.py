        ...
        for record in self.env['hr.employee'].search([]):
            ...
            for contract in Contract.search([('employee_id', '=', record.id)]):
                if contract.date_end:
                    date_end_c = datetime.strptime(contract.date_end, "%Y-%m-%d").date()
            ...
            for insurance in record.insurance_ids:
                if insurance.end_date:
                    date_end_insurance = datetime.strptime(insurance.end_date, "%Y-%m-%d").date()
            ...
            for company in self.env['res.company'].search([]):
                if company.commercial_date:
                    date_end_comm = datetime.strptime(company.commercial_date, "%Y-%m-%d").date()
                ...
                if company.municipal_date:
                    date_end_mu = datetime.strptime(company.municipal_date, "%Y-%m-%d").date()
